...
skip_if_only_changed_value = r'^\.tekton/|\.rhtap$|^docs/|\.md$|^(?:.*/)?(?:\.gitignore|OWNERS|PROJECT|LICENSE)$'
        ...
        for test in data['tests']:
            if 'as' in test:
                if test['as'] in test_steps:
                    ...
                    print(f"Updated skip_if_only_changed in {file_path} for {test['as']}")
                elif 'skip_if_only_changed' in test:
                    del test['skip_if_only_changed']
                    print(f"Removed skip_if_only_changed in {file_path} for {test['as']}")
...
@click.option("--path", type=click.STRING, default="/Users/jiazhu/go/src/github.com/openshift/release", help="the local path to the openshift/release repo")
...
def main(repos, path):
    ...
    for repo in repos:
        ...
        repo_config_dir = os.path.join(path, "ci-operator", "config", "stolostron", repo)
